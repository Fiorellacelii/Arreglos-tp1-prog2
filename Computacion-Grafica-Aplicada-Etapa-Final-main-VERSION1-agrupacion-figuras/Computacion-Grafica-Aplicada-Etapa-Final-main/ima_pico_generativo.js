...
//--- PARES DE FIGURAS VINCULADAS (mutan siempre juntas)
const PARES_VINCULADOS = [
  [F_RECT_AZUL, F_TRIANGULO],
  [F_ARCADA, F_CUADRADO_ROJO],
];

// Formas "primarias" para elegir al azar: solo una por par
// (F_TRIANGULO y F_CUADRADO_ROJO se excluyen porque van a remolque de su par)
const FORMAS_MUTABLES = [F_ZIGZAG, F_ONDA, F_CUARTO_CIRCULO, F_BARRA_AMARILLA,
                         F_RECT_AZUL, F_ARCADA, F_PILDORA, F_MOTIVO];

function parejaVinculada(forma) {
  for (const par of PARES_VINCULADOS) {
    if (par[0] === forma) return par[1];
    if (par[1] === forma) return par[0];
  }
  return null;
}
...
function mutarUnaForma() {
  ...
  // Zigzag lado izquierda = variantes 0,1,3 | derecha = variante 2
  // Onda   lado derecha   = variantes 0,1,3 | izquierda = variante 2
  // Siempre tienen que estar enfrentadas.
  if (forma === F_ZIGZAG) {
    const zigzagDerecha = (varNueva === 2);
    // Si zigzag va a derecha, onda tiene que ir a izquierda (variante 2)
    // Si zigzag va a izquierda, onda tiene que ir a derecha (cualquiera menos 2)
    if (zigzagDerecha && variantes[F_ONDA] !== 2) {
      variantes[F_ONDA]   = 2;
      flashFrames[F_ONDA] = FLASH_DURACION;
    } else if (!zigzagDerecha && variantes[F_ONDA] === 2) {
      variantes[F_ONDA]   = 0;
      flashFrames[F_ONDA] = FLASH_DURACION;
    }
  }
  ...
  if (forma === F_ONDA) {
    const ondaIzquierda = (varNueva === 2);
    // Si onda va a izquierda, zigzag tiene que ir a derecha (variante 2)
    // Si onda va a derecha, zigzag tiene que ir a izquierda (cualquiera menos 2)
    if (ondaIzquierda && variantes[F_ZIGZAG] !== 2) {
      variantes[F_ZIGZAG]   = 2;
      flashFrames[F_ZIGZAG] = FLASH_DURACION;
    } else if (!ondaIzquierda && variantes[F_ZIGZAG] === 2) {
      variantes[F_ZIGZAG]   = 0;
      flashFrames[F_ZIGZAG] = FLASH_DURACION;
    }
  }
}

  // Si la forma tiene pareja, sincronizarla
  const pareja = parejaVinculada(forma);
  if (pareja !== null) {
    variantes[pareja]   = varNueva;
    flashFrames[pareja] = FLASH_DURACION;
  }
